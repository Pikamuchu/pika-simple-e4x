...
function XMLList(array, name) {
  ...
  // Class methods
  this.__get = function(prop) {
    var result = '';
    this.toArray().forEach(function (node) {
      result += node.__get(prop)
    });
    return result;
  };
  this.__set = function(prop, value){
    this.toArray().forEach(function (node) {
      node.__set(prop, value)
    });
  };
...
function XMLBase(obj, name) {
  ...
  this.appendChild = function(child) {
    var node = parseXMLChild(child);
    if (this[node.__name] && this[node.__name].appendChild) {
      this[node.__name].appendChild(node);
    } else {
